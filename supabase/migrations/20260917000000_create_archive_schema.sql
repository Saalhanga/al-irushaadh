...
-- Admin profiles (references auth.users)
...
CREATE POLICY "Anyone can read visible sheikhs"
  ...
  USING (visibility = 'visible' OR auth.uid() IN (SELECT id FROM public.profiles));
...
CREATE POLICY "Anyone can read enabled collections"
  ...
  USING (enabled = true OR auth.uid() IN (SELECT id FROM public.profiles));
...
CREATE POLICY "Anyone can read featured items"
  ON public.featured_items FOR SELECT USING (enabled = true OR auth.uid() IN (SELECT id FROM public.profiles));
